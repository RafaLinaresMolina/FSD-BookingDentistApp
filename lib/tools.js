...
const userFound = async (id, collection) => {
  try {
    const Model = getModelBycollection(collection)
    process.log.debug(" -> tools.userFound");
    const userDoc = await Model.findOne({ _id: id, status: { $ne: 0 } });
    if (!userDoc) {
      process.log.warning(" <- tools.userFound");
      throw new Error(`User with Id '${id}' not found`);
    }
    process.log.debug(" <- tools.userFound");
    return userDoc;
  } catch (err) {
    process.log.error(` x- tools.userFound: ${err.message}`);
    throw err;
  }
};
...
const getModelBycollection = (collection) => {
  ...
  return models[collection] ? models[collection] : UserModel;
};
